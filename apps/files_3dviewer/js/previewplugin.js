...
(function(OCA) {
	...
	OCA.Files3dViewer.PreviewPlugin = {
		...
		show: function(downloadUrl, isFileList) {
			var self = this;
			var $iframe;
			var viewer = OC.generateUrl('/apps/files_3dviewer/?file={file}', {file: downloadUrl});
		...
		_extendFileActions: function(fileActions) {
			...
			fileActions.setDefault('application/wexbim', 'view');
		}
	};
...
$(document).ready(function(){
	// Doesn't work with IE below 9
	if(!$.browser.msie || ($.browser.msie && $.browser.version >= 9)){
		if ($('#isPublic').val() && $('#mimetype').val() === 'application/wexbim') {
			var sharingToken = $('#sharingToken').val();
			var downloadUrl = OC.generateUrl('/s/{token}/download', {token: sharingToken});
			var viewer = OCA.Files3dViewer.PreviewPlugin;
